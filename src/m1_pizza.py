...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  80 purple dots on a black circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3: 5 segments with medium black lines'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'black', 5)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points)):
        segment = rg.Line(points[k - 1], points[k])
        segment.thickness = thickness
        segment.color = color
        segment.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 3:  50 blue lines on green circle, hops = 7.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'green'
    fancy_polygon(window, circle, 50, 7, 'blue', 3)
    window.close_on_mouse_click()
